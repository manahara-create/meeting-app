...
const ExportButton = ({ 
    ...
    const exportToExcel = () => {
        try {
            const dataForExport = activities.map(activity => {
                // Common structure for Cluster 4 modules
                const base = {
    ...
    const exportToPDF = () => {
        try {
            const doc = new jsPDF();
            doc.setFontSize(16);
            doc.text('Cluster 4 Export Summary', 14, 15);
            doc.setFontSize(10);
            ...
            const fileName = `${selectedCategory?.name || 'cluster_4_export'}_${dayjs().format('YYYY-MM-DD')}.pdf`;
...
const Cluster4 = () => {
    ...
    // Updated Cluster 4 Categories configuration to match your database structure
    ...
    const fetchCluster4Users = async () => {
        try {
            // First get the Cluster 4 department ID
            ...
            if (deptError) {
                // If Cluster 4 department doesn't exist, try case-insensitive search
            ...
            // Then get all users in Cluster 4 department
        ...
        try {
            ...
            for (const category of Cluster4Categories) {
                ...
                try {
                    ...
                    if (data) {
                        // For Cluster 4, we'll include all activities since there's no specific user assignment
