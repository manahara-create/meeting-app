...
const ErrorFallback = ({ error, resetErrorBoundary }) => (
    <Result
        status="error"
        title="Something went wrong in Regulatory Module"
...
// Loading component
const LoadingSpinner = ({ tip = "Loading Regulatory data..." }) => (
...
const RegulatoryStatistics = ({ stats, loading = false }) => (
...
const DiscussionModal = React.memo(({
    ...
    const getFeedbackTable = useCallback(() => {
        ...
        const tableMap = {
            'meetings': 'regulatory_meetings_fb',
            'submissions': 'regulatory_submissions_fb'
        };
                    ...
                    <>
                        ...
                        >
                            <TimePicker
                                ...
                                onChange={(time) => updateRecord(index, 'start_time', time ? time.format('HH:mm') : null)}
                                style={{ width: '100%' }}
                                format="HH:mm"
                        ...
                        >
                            <TimePicker
                                ...
                                onChange={(time) => updateRecord(index, 'end_time', time ? time.format('HH:mm') : null)}
                                style={{ width: '100%' }}
                                format="HH:mm"
                    ...
                    <>
                        ...
                        >
                            <Input
                                ...
                                onChange={(e) => updateRecord(index, 'product', e.target.value)}
                        ...
                        >
                            <Input
                                ...
                                onChange={(e) => updateRecord(index, 'principal', e.target.value)}
                        ...
                        >
                            <DatePicker
                                value={record.expire_date ? safeDayjs(record.expire_date) : null}
                                onChange={(date) => updateRecord(index, 'expire_date', date)}
                                style={{ width: '100%' }}
                                format="DD/MM/YYYY"
...
const ExcelImportModal = ({ visible, onCancel, selectedCategory, onImportComplete, allProfiles }) => {
    ...
    const downloadTemplate = () => {
        try {
            ...
            const fileName = `${selectedCategory?.name || 'regulatory'}_import_template.xlsx`;
...
const ExportButton = ({ 
    ...
    const exportToExcel = () => {
        try {
            const dataForExport = activities.map(activity => {
                // Common structure shared by Regulatory modules
                const base = {
                ...
                switch (selectedCategory?.id) {
                    ...
                    case 'submissions':
                        return {
                            ...
                            'Expire Date': activity.expire_date
                                ? dayjs(activity.expire_date).format('YYYY-MM-DD')
                                : ''
                        };
            ...
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Regulatory Export');
            ...
            const fileName = `${selectedCategory?.name || 'regulatory_export'}_${dayjs().format('YYYY-MM-DD')}.xlsx`;
    ...
    const exportToWord = async () => {
        try {
                ...
                switch (selectedCategory?.id) {
                    case 'meetings':
                        cells.push(
                            ...
                            new TableCell({ children: [new Paragraph(activity.start_time || '')] }),
                    ...
                    case 'submissions':
                        cells.push(
                            ...
                            new TableCell({ children: [new Paragraph(activity.product || '')] }),
                            new TableCell({ children: [new Paragraph(activity.principal || '')] }),
                            new TableCell({ children: [new Paragraph(activity.date ? dayjs(activity.date).format('DD/MM/YYYY') : '')] }),
                            new TableCell({ children: [new Paragraph(activity.expire_date ? dayjs(activity.expire_date).format('DD/MM/YYYY') : '')] }),
                            new TableCell({ children: [new Paragraph(getPriorityLabel(activity.priority))] })
                        );
                        break;
            ...
            switch (selectedCategory?.id) {
                case 'meetings':
                    headers = ['Subject', 'Type', 'Date', 'Start Time', 'Status', 'Priority'];
                    break;
                case 'submissions':
                    headers = ['Company', 'Product', 'Principal', 'Date', 'Expire Date', 'Priority'];
                    break;
                default:
                    headers = ['Type', 'Priority'];
            }
                ...
                sections: [{
                    ...
                    children: [
                        new Paragraph({
                            children: [
                                new TextRun({
                                    text: `${selectedCategory?.name || 'Regulatory'} Export - ${dayjs().format('DD/MM/YYYY')}`,
                                    bold: true,
                                    size: 28,
                                }),
                            ],
            ...
            link.href = url;
            link.download = `${selectedCategory?.name || 'regulatory_export'}_${dayjs().format('YYYY-MM-DD')}.docx`;
...
const Regulatory = () => {
    ...
    // Regulatory Categories configuration
    const regulatoryCategories = [
        {
            id: 'meetings',
            name: 'Meetings',
            table: 'regulatory_meetings',
            type: 'Meeting',
    ...
    const refreshRegulatoryData = async () => {
        ...
        try {
            ...
            toast.info('Regulatory data updated automatically');
        } catch (error) {
    ...
    const manualRefresh = async () => {
        ...
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.success('Regulatory data refreshed successfully');
        } catch (error) {
    ...
    // Initialize Regulatory module
    const initializeRegulatory = async () => {
        setLoading(true);
        try {
            ...
            handleError(error, 'initializing Regulatory module');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchTableData = async () => {
        ...
        try {
            ...
            let query = supabase
                .from(selectedCategory.table)
                .select('*')
                .eq('department_id', '6e087a71-6f40-49c8-b69d-e435e3a06279') // Regulatory department ID
    ...
    const handleEdit = (record) => {
        try {
            ...
            try {
                ...
                if (selectedCategory.id === 'submissions') {
                    if (record.date) {
                        formattedRecord.date = safeDayjs(record.date);
                    }
                    if (record.expire_date) {
                        formattedRecord.expire_date = safeDayjs(record.expire_date);
                    }
                }
            } catch (dateError) {
        ...
        try {
            ...
            const validRecords = records.filter(record => {
                ...
                switch (selectedCategory.id) {
                    ...
                    case 'submissions':
                        return record.date && record.company;
                    default:
                        return true;
                }
            });
                    ...
                    return (
                        <>
                            ...
                            <Form.Item
                                ...
                                label="Principal"
                            >
                                <Input placeholder="Enter principal" />
                            </Form.Item>
                            <Form.Item
                                name="expire_date"
                                label="Expire Date"
    ...
    if (loading && !selectedCategory) {
        return <LoadingSpinner tip="Loading Regulatory module..." />;
    }
    ...
    return (
        <div style={{ padding: '16px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            {error && (
                <Alert
                    message="Regulatory Module Error"
                ...
                <Row justify="space-between" align="middle" gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={8}>
                        <Title level={2} style={{ margin: 0, fontSize: '24px' }}>
                            <SafetyCertificateOutlined /> Regulatory Department
                        </Title>
                    </Col>
                    <Col xs={24} sm={12} md={8}>
                        <Space direction="vertical" style={{ width: '100%' }} size="small">
            ...
            {autoRefresh && (
                <Alert
                    ...
                    description="Regulatory data will automatically update every 2 minutes."
                    type="info"
                    showIcon
                    closable
                    style={{ marginBottom: 16 }}
            ...
            <Card
                ...
                extra={
                    <Tag color="blue">
                        {regulatoryCategories.length} Categories Available
                    </Tag>
                }
            >
                <Row gutter={[16, 16]}>
            ...
            {selectedCategory && dateRange[0] && dateRange[1] && (
                <>
                    <RegulatoryStatistics stats={stats} loading={loading} />
            ...
            {/* Instructions */}
            {!selectedCategory && (
                <Card title="How to Use Regulatory Module" style={{ borderRadius: '12px' }}>
                    <Alert
                        message="Manage Regulatory Department Data"
                        description={
                            <div>
                                ...
                                <Text type="secondary">
                                    Each category represents different Regulatory activities recorded in the system.
