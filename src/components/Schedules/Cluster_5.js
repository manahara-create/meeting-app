...
const ExcelImportModal = ({ visible, onCancel, selectedCategory, onImportComplete, profiles }) => {
    ...
    const downloadTemplate = () => {
        try {
            ...
            const fileName = `${selectedCategory?.name || 'cluster_5'}_import_template.xlsx`;
...
const ExportButton = ({
    ...
    const exportToWord = async () => {
        try {
                ...
                sections: [{
                    ...
                    children: [
                        new Paragraph({
                            children: [
                                new TextRun({
                                    text: `Cluster 5 Export - ${selectedCategory?.name}`,
                                    bold: true,
                                    size: 32
                                })
                            ],
            ...
            link.href = url;
            link.download = `${selectedCategory?.name || 'cluster_5_export'}_${dayjs().format('YYYY-MM-DD')}.docx`;
...
const Cluster5 = () => {
    ...
    // Cluster 5 Categories configuration
    ...
    const fetchCluster5Users = async () => {
        try {
            ...
            // Get all users in Cluster 5 department
                ...
                <Row justify="space-between" align="middle" gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={8}>
                        <Title level={2} style={{ margin: 0, fontSize: '24px' }}>
                            <SafetyCertificateOutlined /> Cluster 5 Department
                        </Title>
                    </Col>
                    <Col xs={24} sm={12} md={8}>
                        <Space direction="vertical" style={{ width: '100%' }} size="small">
