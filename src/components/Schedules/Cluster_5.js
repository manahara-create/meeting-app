...
const ErrorFallback = ({ error, resetErrorBoundary }) => (
    <Result
        status="error"
        title="Something went wrong in Cluster 5 Module"
...
// Loading component
const LoadingSpinner = ({ tip = "Loading Cluster 5 data..." }) => (
...
const Cluster5Statistics = ({ stats, loading = false }) => (
...
const DiscussionModal = React.memo(({
    ...
    // Get the correct feedback table name for Cluster 5
    const getFeedbackTable = useCallback(() => {
        if (!category) return null;

        const tableMap = {
            'visit_plan': 'cluster_5_visit_plan_fb',
            'meetings': 'cluster_5_meetings_fb',
            'special_task': 'cluster_5_special_task_fb'
        };
...
const UserScheduleModal = React.memo(({
    ...
    const getActivityType = (item) => {
        try {
            ...
            if (item.type === 'cluster_5_activity') return `Cluster 5 ${item.activity_type}`;
    ...
    const getActivityDescription = (item) => {
        try {
            ...
            if (item.type === 'cluster_5_activity') {
...
const ExportButton = ({ 
    ...
    const exportToExcel = () => {
        try {
            const dataForExport = activities.map(activity => {
                // Common structure for Cluster 5 modules
                const base = {
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Cluster 5 Export');
            ...
            const fileName = `${selectedCategory?.name || 'cluster_5_export'}_${dayjs().format('YYYY-MM-DD')}.xlsx`;
    ...
    const exportToPDF = () => {
        try {
            const doc = new jsPDF();
            doc.setFontSize(16);
            doc.text('Cluster 5 Export Summary', 14, 15);
            doc.setFontSize(10);
            ...
            const fileName = `${selectedCategory?.name || 'cluster_5_export'}_${dayjs().format('YYYY-MM-DD')}.pdf`;
...
const Cluster5 = () => {
    ...
    const [Cluster5Users, setCluster5Users] = useState([]);
    ...
    // Updated Cluster 5 Categories configuration to match your database structure
    const Cluster5Categories = [
        {
            id: 'visit_plan',
            name: 'Visit Plan',
            table: 'cluster_5_visit_plan',
            type: 'Task',
    ...
    const refreshCluster5Data = async () => {
        ...
        try {
            ...
            toast.info('Cluster 5 data updated automatically');
        } catch (error) {
    ...
    const manualRefresh = async () => {
        ...
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.success('Cluster 5 data refreshed successfully');
        } catch (error) {
    ...
    // Initialize Cluster 5 module
    const initializeCluster5 = async () => {
        setLoading(true);
        try {
            ...
            handleError(error, 'initializing Cluster 5 module');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchCluster5Users = async () => {
        try {
            // First get the Cluster 5 department ID
            ...
            if (deptError) {
                // If Cluster 5 department doesn't exist, try case-insensitive search
                const { data: deptDataAlt, error: deptErrorAlt } = await supabase
                    .from('departments')
                    .select('id')
                    .ilike('name', '%Cluster 5%')
                    .single();

                if (deptErrorAlt) throw deptErrorAlt;
                if (!deptDataAlt) {
                    toast.warning('Cluster 5 department not found. Using all users as fallback.');
                    ...
                    safeSetState(setCluster5Users, allUsers || []);
                    return;
                }

                safeSetState(setCluster5Users, []);
                return;
            }

            if (!deptData) {
                toast.warning('Cluster 5 department not found');
                safeSetState(setCluster5Users, []);
                return;
            }

            // Then get all users in Cluster 5 department
            ...
            if (usersError) throw usersError;
            safeSetState(setCluster5Users, usersData || []);
        } catch (error) {
            handleError(error, 'fetching Cluster 5 users');
            safeSetState(setCluster5Users, []);
        }
    };
    ...
    const fetchUserSchedule = async (userId, startDate, endDate) => {
        ...
        try {
            ...
            // Get user details
            const user = Cluster5Users.find(u => u.id === userId);
            if (!user) {
                console.warn('User not found in Cluster 5 users list');
            ...
            // 2. Cluster 5 activities
            for (const category of Cluster5Categories) {
                ...
                try {
                    ...
                    if (data) {
                        // For Cluster 5, we'll include all activities since there's no specific user assignment
    ...
    if (loading && !selectedCategory) {
        return <LoadingSpinner tip="Loading Cluster 5 module..." />;
    }
    ...
    return (
        <div style={{ padding: '16px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            {error && (
                <Alert
                    message="Cluster 5 Module Error"
                ...
                <Row justify="space-between" align="middle" gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={8}>
                        <Title level={2} style={{ margin: 0, fontSize: '24px' }}>
                            <TeamOutlined /> Cluster 5 Department
                        </Title>
                    </Col>
                    <Col xs={24} sm={12} md={8}>
            ...
            {autoRefresh && (
                <Alert
                    ...
                    description="Cluster 5 data will automatically update every 2 minutes."
                    type="info"
                    showIcon
                    closable
                    style={{ marginBottom: 16 }}
            ...
            {/* Category Cards */}
            <Card
                title="Cluster 5 Categories"
                style={{ marginBottom: 24 }}
                extra={
                    <Tag color="blue">
                        {Cluster5Categories.length} Categories Available
                    </Tag>
                }
            >
                <Row gutter={[16, 16]}>
                    {Cluster5Categories.map((category) => (
            ...
            {selectedCategory && dateRange[0] && dateRange[1] && (
                <>
                    <Cluster5Statistics stats={stats} loading={loading} />
            ...
            <Modal
                title={
                    <Space>
                        <UserOutlined />
                        Check Cluster 5 Team Availability
                        <Tag color="blue">
            ...
            >
                <Space direction="vertical" style={{ width: '100%' }} size="large">
                    ...
                    {/* Cluster 5 Users List */}
                    <Card size="small" title="Cluster 5 Team Members">
                        {Cluster5Users.length === 0 ? (
                            <Empty
                                image={Empty.PRESENTED_IMAGE_SIMPLE}
                                description="No Cluster 5 team members found"
                            />
                        ) : (
                            <List
                                dataSource={Cluster5Users}
                                renderItem={user => (
                                    <List.Item
                                        actions={[
                                            <Tooltip
            ...
            {/* Instructions */}
            {!selectedCategory && (
                <Card title="How to Use Cluster 5 Module" style={{ borderRadius: '12px' }}>
                    <Alert
                        message="Manage Cluster 5 Department Data"
                        description={
                            <div>
                                ...
                                <Text type="secondary">
                                    Each category represents different Cluster 5 activities recorded in the system.
