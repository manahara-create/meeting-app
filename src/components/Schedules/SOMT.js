...
const ErrorFallback = ({ error, resetErrorBoundary }) => (
    <Result
        status="error"
        title="Something went wrong in SOMT Module"
...
// Loading component
const LoadingSpinner = ({ tip = "Loading SOMT data..." }) => (
...
const SOMTStatistics = ({ stats, loading = false }) => (
...
const DiscussionModal = React.memo(({
    ...
    const getFeedbackTable = useCallback(() => {
        ...
        const tableMap = {
            'meetings': 'somt_meetings_fb',
            'tender': 'somt_tender_fb'
        };
    ...
    return (
        <Modal
            title={
                <Space>
                    <WechatOutlined />
                    Discussion: {record?.subject || record?.customer || 'Record'}
                    {record?.date && ` - ${dayjs(record.date).format('DD/MM/YYYY')}`}
                    {record?.close_date && ` - ${dayjs(record.close_date).format('DD/MM/YYYY')}`}
...
const ExcelImportModal = ({ visible, onCancel, selectedCategory, onImportComplete, allProfiles }) => {
    ...
    const validateExcelData = (data) => {
        ...
        data.forEach((row, index) => {
            ...
            const responsibleSalesPersonColumns = [
                'responsible_sales_person', 'responsible_sales_person_ids', 'responsible_sales_person_names',
                'Responsible Sales Person', 'Responsible Sales Persons', 'Sales Person Responsible'
            ];

            const foundSalesPersonColumns = responsibleSalesPersonColumns.filter(col => 
                row.hasOwnProperty(col) && row[col] !== undefined && row[col] !== null && row[col] !== ''
            );

            if (foundSalesPersonColumns.length > 0) {
                warnings.push(`Responsible Sales Person columns (${foundSalesPersonColumns.join(', ')}) are ignored. Use Bulk Records feature to assign sales persons.`);
            }
    ...
    const importData = async () => {
        ...
        try {
            ...
            const warningMessage = hasResponsibleSalesPersonField 
                ? `Successfully imported ${data.length} records. Use Bulk Records feature to assign Responsible Sales Persons.`
        ...
        >
            <Space direction="vertical" style={{ width: '100%' }} size="large">
                ...
                {hasResponsibleSalesPersonField && (
                    <Alert
                        message="Important Notice: Responsible Sales Person Assignment"
                        description={
                            <div>
                                <Text strong style={{ color: '#fa8c16' }}>
                                    Responsible Sales Person assignments cannot be set via Excel import.
                                </Text>
                                <ul style={{ margin: '8px 0', paddingLeft: '20px' }}>
                                    <li>Excel import will ignore any Responsible Sales Person columns</li>
                                    <li>After import, use the Bulk Records feature to assign Responsible Sales Persons</li>
                    ...
                    description={
                        <ul style={{ margin: 0, paddingLeft: '16px' }}>
                            ...
                            {hasResponsibleSalesPersonField && (
                                <>
                                    <li>
                                        <Text strong>Responsible Sales Person assignments will be ignored in Excel import</Text>
                                    </li>
                                    <li>Use the Bulk Records feature to assign Responsible Sales Persons</li>
                                </>
                            )}
                        </ul>
                    }
...
const ExportButton = ({ 
    ...
    const exportToExcel = () => {
        try {
            ...
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, 'SOMT Export');
            ...
            const fileName = `${selectedCategory?.name || 'somt_export'}_${dayjs().format('YYYY-MM-DD')}.xlsx`;
...
const SOMT = () => {
    ...
    const refreshSOMTData = async () => {
        ...
        try {
            ...
            toast.info('SOMT data updated automatically');
        } catch (error) {
    ...
    const manualRefresh = async () => {
        ...
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.success('SOMT data refreshed successfully');
        } catch (error) {
    ...
    const initializeSOMT = async () => {
        ...
        try {
            ...
            handleError(error, 'initializing SOMT module');
        } finally {
            setLoading(false);
        }
    };
    ...
    if (loading && !selectedCategory) {
        return <LoadingSpinner tip="Loading SOMT module..." />;
    }
        ...
        <div style={{ padding: '16px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            {autoRefresh && (
                <Alert
                    ...
                    description="SOMT data will automatically update every 2 minutes."
                    type="info"
                    showIcon
                    closable
                    style={{ marginBottom: 16 }}
            ...
            {/* Category Cards */}
            <Card
                title="SOMT Categories"
                style={{ marginBottom: 24 }}
                extra={
                    <Tag color="blue">
                        {somtCategories.length} Categories Available
                    </Tag>
                }
            >
                <Row gutter={[16, 16]}>
                    {somtCategories.map((category) => (
            ...
            {selectedCategory && dateRange[0] && dateRange[1] && (
                <>
                    <SOMTStatistics stats={stats} loading={loading} />
            ...
            {/* Instructions */}
            {!selectedCategory && (
                <Card title="How to Use SOMT Module" style={{ borderRadius: '12px' }}>
                    <Alert
                        message="Manage SOMT Department Data"
                        description={
                            <div>
