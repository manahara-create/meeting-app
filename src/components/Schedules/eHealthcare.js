...
const ErrorFallback = ({ error, resetErrorBoundary }) => (
    <Result
        status="error"
        title="Something went wrong in eHealthcare Module"
...
// Loading component
const LoadingSpinner = ({ tip = "Loading eHealthcare data..." }) => (
...
const EHealthcareStatistics = ({ stats, loading = false }) => (
...
const DiscussionModal = React.memo(({
    ...
    // Get the correct feedback table name for eHealthcare
    const getFeedbackTable = useCallback(() => {
        if (!category) return null;

        const tableMap = {
            'visit_plan': 'ehealthcare_visit_plan_fb',
            'meetings': 'ehealthcare_meetings_fb'
        };
    ...
    return (
        <Modal
            title={
                <Space>
                    <WechatOutlined />
                    Discussion: {record?.subject || record?.name || 'Record'}
...
const UserScheduleModal = React.memo(({
    ...
    const getActivityType = (item) => {
        try {
            ...
            if (item.type === 'ehealthcare_activity') return `eHealthcare ${item.activity_type}`;
    ...
    const getActivityDescription = (item) => {
        try {
            ...
            if (item.type === 'ehealthcare_activity') {
                return item.subject || item.purpose || 'No description available';
            }
                                ...
                                <div style={{ padding: '8px 0' }}>
                                    ...
                                    >
                                        ...
                                        <Descriptions.Item label="Title">
                                            <Text strong>
                                                {item.subject || (Array.isArray(item.name) ? item.name.join(', ') : item.name) || 'Unknown Activity'}
                                            </Text>
                                        </Descriptions.Item>
...
const EHealthcare = () => {
    ...
    const [ehealthcareUsers, setEHealthcareUsers] = useState([]);
    ...
    const refreshEHealthcareData = async () => {
        ...
        try {
            ...
            toast.info('eHealthcare data updated automatically');
        } catch (error) {
    ...
    const manualRefresh = async () => {
        ...
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.success('eHealthcare data refreshed successfully');
        } catch (error) {
    ...
    // Initialize eHealthcare module
    const initializeEHealthcare = async () => {
        setLoading(true);
        try {
            ...
            handleError(error, 'initializing eHealthcare module');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchUserSchedule = async (userId, startDate, endDate) => {
        ...
        try {
            ...
            // Get user details
            const user = ehealthcareUsers.find(u => u.id === userId);
            if (!user) {
                console.warn('User not found in eHealthcare users list');
            ...
            // 2. eHealthcare activities
            for (const category of ehealthcareCategories) {
    ...
    if (loading && !selectedCategory) {
        return <LoadingSpinner tip="Loading eHealthcare module..." />;
    }
    ...
    return (
        <div style={{ padding: '16px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            {error && (
                <Alert
                    message="eHealthcare Module Error"
                ...
                <Row justify="space-between" align="middle" gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={8}>
                        <Title level={2} style={{ margin: 0, fontSize: '24px' }}>
                            <MedicineBoxOutlined /> eHealthcare Department
                        </Title>
                    </Col>
                    <Col xs={24} sm={12} md={8}>
            ...
            {autoRefresh && (
                <Alert
                    ...
                    description="eHealthcare data will automatically update every 2 minutes."
                    type="info"
                    showIcon
                    closable
                    style={{ marginBottom: 16 }}
            ...
            {/* Category Cards */}
            <Card
                title="eHealthcare Categories"
                style={{ marginBottom: 24 }}
                extra={
                    <Tag color="blue">
                        {ehealthcareCategories.length} Categories Available
                    </Tag>
                }
            >
                <Row gutter={[16, 16]}>
                    {ehealthcareCategories.map((category) => (
            ...
            {selectedCategory && dateRange[0] && dateRange[1] && (
                <>
                    <EHealthcareStatistics stats={stats} loading={loading} />
            ...
            <Modal
                title={
                    <Space>
                        <UserOutlined />
                        Check eHealthcare Team Availability
                        <Tag color="blue">
            ...
            >
                <Space direction="vertical" style={{ width: '100%' }} size="large">
                    ...
                    {/* eHealthcare Users List */}
                    <Card size="small" title="eHealthcare Team Members">
                        {ehealthcareUsers.length === 0 ? (
                            <Empty
                                image={Empty.PRESENTED_IMAGE_SIMPLE}
                                description="No eHealthcare team members found"
                            />
                        ) : (
                            <List
                                dataSource={ehealthcareUsers}
                                renderItem={user => (
                                    <List.Item
                                        actions={[
                                            <Tooltip
            ...
            {/* Instructions */}
            {!selectedCategory && (
                <Card title="How to Use eHealthcare Module" style={{ borderRadius: '12px' }}>
                    <Alert
                        message="Manage eHealthcare Department Data"
                        description={
                            <div>
