...
const ErrorFallback = ({ error, resetErrorBoundary }) => (
    <Result
        status="error"
        title="Something went wrong in Senior Management Module"
...
// Loading component
const LoadingSpinner = ({ tip = "Loading Senior Management data..." }) => (
...
const SeniorManagementStatistics = ({ stats, loading = false }) => (
...
const DiscussionModal = React.memo(({
    ...
    // Get the correct feedback table name for Senior Management
    const getFeedbackTable = useCallback(() => {
        if (!category) return null;

        const tableMap = {
            'meetings': 'senior_management_meetings_fb',
            'special_task': 'senior_management_special_task_fb'
        };
...
const UserScheduleModal = React.memo(({
        ...
        try {
            switch (item.type) {
                case 'personal_meeting':
                    return 'blue';
                case 'senior_management_activity':
                    return 'green';
                default:
                    return 'gray';
            }
    ...
    const getActivityType = (item) => {
        try {
            ...
            if (item.type === 'senior_management_activity') return `Senior Management ${item.activity_type}`;
            return 'Activity';
        } catch (error) {
            return 'Activity';
        }
    ...
    const getActivityDescription = (item) => {
        try {
            ...
            if (item.type === 'senior_management_activity') {
...
const ExcelImportModal = ({ visible, onCancel, selectedCategory, onImportComplete, profiles }) => {
    ...
    const downloadTemplate = () => {
        try {
            ...
            const fileName = `${selectedCategory?.name || 'senior_management'}_import_template.xlsx`;
...
const ExportButton = ({
    ...
    const exportToExcel = () => {
        try {
            ...
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Senior Management Export');
            ...
            const fileName = `${selectedCategory?.name || 'senior_management_export'}_${dayjs().format('YYYY-MM-DD')}.xlsx`;
    ...
    const exportToWord = async () => {
        try {
                ...
                sections: [{
                    ...
                    children: [
                        new Paragraph({
                            children: [
                                new TextRun({
                                    text: `Senior Management Export - ${selectedCategory?.name}`,
                                    bold: true,
                                    size: 32
                                })
                            ],
            ...
            link.href = url;
            link.download = `${selectedCategory?.name || 'senior_management_export'}_${dayjs().format('YYYY-MM-DD')}.docx`;
...
const SeniorManagement = () => {
    ...
    const [SeniorManagementUsers, setSeniorManagementUsers] = useState([]);
    ...
    // Senior Management Categories configuration
    const SeniorManagementCategories = [
        {
            id: 'meetings',
            name: 'Meetings',
            table: 'senior_management_meetings',
            type: 'Meeting',
            icon: <TeamOutlined />,
            dateField: 'date',
            color: '#fa8c16',
        ...
        },
        {
            id: 'special_task',
            name: 'Special Task',
            table: 'senior_management_special_task',
            type: 'Special Task',
    ...
    const refreshSeniorManagementData = async () => {
        ...
        try {
            ...
            toast.info('Senior Management data updated automatically');
        } catch (error) {
    ...
    const manualRefresh = async () => {
        ...
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.success('Senior Management data refreshed successfully');
        } catch (error) {
    ...
    // Initialize Senior Management module
    const initializeSeniorManagement = async () => {
        setLoading(true);
        try {
            await Promise.allSettled([
                fetchCurrentUser(),
                fetchProfiles(),
                fetchSeniorManagementUsers()
            ]);
            ...
            handleError(error, 'initializing Senior Management module');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchSeniorManagementUsers = async () => {
        try {
            // Get Senior Management department ID
            const { data: deptData, error: deptError } = await supabase
                .from('departments')
                .select('id')
                .eq('name', 'Senior Management')
                .single();
            ...
            if (deptError) {
                ...
                const { data: deptDataAlt, error: deptErrorAlt } = await supabase
                    .from('departments')
                    .select('id')
                    .ilike('name', '%Senior Management%')
                    .single();

                if (deptErrorAlt) throw deptErrorAlt;
                if (!deptDataAlt) {
                    toast.warning('Senior Management department not found. Using all users as fallback.');
                    ...
                    safeSetState(setSeniorManagementUsers, allUsers || []);
                    return;
                }

                safeSetState(setSeniorManagementUsers, []);
                return;
            }

            if (!deptData) {
                toast.warning('Senior Management department not found');
                safeSetState(setSeniorManagementUsers, []);
                return;
            }

            // Get all users in Senior Management department
            ...
            if (usersError) throw usersError;
            safeSetState(setSeniorManagementUsers, usersData || []);
        } catch (error) {
            handleError(error, 'fetching Senior Management users');
            safeSetState(setSeniorManagementUsers, []);
        }
    };
    ...
    const fetchUserSchedule = async (userId, startDate, endDate) => {
        ...
        try {
            ...
            // Get user details
            const user = SeniorManagementUsers.find(u => u.id === userId);
            if (!user) {
                console.warn('User not found in Senior Management users list');
            ...
            // 2. Senior Management activities
            for (const category of SeniorManagementCategories) {
                ...
                try {
                    ...
                    if (categoryActivities.length > 0) {
                        allActivities.push(...categoryActivities.map(activity => ({
                            ...activity,
                            type: 'senior_management_activity',
    ...
    if (loading && !selectedCategory) {
        return <LoadingSpinner tip="Loading Senior Management module..." />;
    }
    ...
    return (
        <div style={{ padding: '16px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            {error && (
                <Alert
                    message="Senior Management Module Error"
                ...
                <Row justify="space-between" align="middle" gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={8}>
                        <Title level={2} style={{ margin: 0, fontSize: '24px' }}>
                            <SafetyCertificateOutlined /> Senior Management Department
                        </Title>
                    </Col>
                    <Col xs={24} sm={12} md={8}>
                        <Space direction="vertical" style={{ width: '100%' }} size="small">
            ...
            {autoRefresh && (
                <Alert
                    ...
                    description="Senior Management data will automatically update every 2 minutes."
                    type="info"
                    showIcon
                    closable
                    style={{ marginBottom: 16 }}
            ...
            {/* Category Cards */}
            <Card
                title="Senior Management Categories"
                style={{ marginBottom: 24 }}
                extra={
                    <Tag color="blue">
                        {SeniorManagementCategories.length} Categories Available
                    </Tag>
                }
            >
                <Row gutter={[16, 16]}>
                    {SeniorManagementCategories.map((category) => (
            ...
            {selectedCategory && dateRange[0] && dateRange[1] && (
                <>
                    <SeniorManagementStatistics stats={stats} loading={loading} />
            ...
            <Modal
                title={
                    <Space>
                        <UserOutlined />
                        Check Senior Management Team Availability
                        <Tag color="blue">
            ...
            >
                <Space direction="vertical" style={{ width: '100%' }} size="large">
                    ...
                    {/* Senior Management Users List */}
                    <Card size="small" title="Senior Management Team Members">
                        {SeniorManagementUsers.length === 0 ? (
                            <Empty
                                image={Empty.PRESENTED_IMAGE_SIMPLE}
                                description="No Senior Management team members found"
                            />
                        ) : (
                            <List
                                dataSource={SeniorManagementUsers}
                                renderItem={user => (
                                    <List.Item
                                        actions={[
                                            <Tooltip
                                    ...
                                    >
                                        <List.Item.Meta
                                            ...
                                            description={
                                                <Badge
                                                    status="success"
                                                    text="Senior Management Team Member"
                                                />
                                            }
                                        />
                                    </List.Item>
            ...
            {/* Instructions */}
            {!selectedCategory && (
                <Card title="How to Use Senior Management Module" style={{ borderRadius: '12px' }}>
                    <Alert
                        message="Manage Senior Management Department Data"
                        description={
                            <div>
                                ...
                                <Text type="secondary">
                                    Each category represents different Senior Management activities recorded in the system.
