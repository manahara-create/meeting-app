...
const ErrorFallback = ({ error, resetErrorBoundary }) => (
    <Result
        status="error"
        title="Something went wrong in Customer Care Module"
...
// Loading component
const LoadingSpinner = ({ tip = "Loading Customer Care data..." }) => (
...
const CustomerCareStatistics = ({ stats, loading = false }) => (
...
const DiscussionModal = React.memo(({
    ...
    // Get the correct feedback table name for Customer Care
    const getFeedbackTable = useCallback(() => {
        if (!category) return null;

        const tableMap = {
            'delivery_schedule': 'customer_care_delivery_schedule_fb',
            'meetings': 'customer_care_meetings_fb',
            'special_tasks': 'customer_care_special_tasks_fb'
        };
    ...
    return (
        <Modal
            title={
                <Space>
                    <WechatOutlined />
                    Discussion: {record?.subject || record?.region || 'Record'}
                    {record?.date && ` - ${dayjs(record.date).format('DD/MM/YYYY')}`}
                    {record?.delivery_date && ` - ${dayjs(record.delivery_date).format('DD/MM/YYYY')}`}
...
const UserScheduleModal = React.memo(({
    ...
    const getActivityType = (item) => {
        try {
            ...
            if (item.type === 'customer_care_activity') return `Customer Care ${item.activity_type}`;
            return 'Activity';
        } catch (error) {
            return 'Activity';
        }
    ...
    const getActivityDescription = (item) => {
        try {
            ...
            if (item.type === 'customer_care_activity') {
                return item.subject || item.region || 'No description available';
            }
                                ...
                                <div style={{ padding: '8px 0' }}>
                                    ...
                                    >
                                        ...
                                        <Descriptions.Item label="Title">
                                            <Text strong>
                                                {item.subject || item.region || 'Activity'}
                                            </Text>
                                        </Descriptions.Item>
                                        <Descriptions.Item label="Date">
                                            <Space>
                                                <CalendarOutlined />
                                                {safeDayjs(item.date || item.delivery_date).format('DD/MM/YYYY')}
                                            </Space>
                                        </Descriptions.Item>
            ...
            case 'delivery_schedule':
                return (
                    <>
                        ...
                        >
                            <DatePicker
                                value={record.delivery_date ? safeDayjs(record.delivery_date) : null}
                                onChange={(date) => updateRecord(index, 'delivery_date', date)}
                                style={{ width: '100%' }}
                                format="DD/MM/YYYY"
                        ...
                        >
                            <Input
                                value={record.region || ''}
                                onChange={(e) => updateRecord(index, 'region', e.target.value)}
                        ...
                        >
                            <Input
                                ...
                                onChange={(e) => updateRecord(index, 'duration', e.target.value)}
    ...
    const exportToExcel = () => {
        try {
            const dataForExport = activities.map(activity => {
                // Common structure for Customer Care modules
                const base = {
                ...
                switch (selectedCategory?.id) {
                    case 'delivery_schedule':
                        return {
                            ...
                            'Area': activity.area || '',
                            'Delivery Date': activity.delivery_date
                                ? dayjs(activity.delivery_date).format('YYYY-MM-DD')
                                : '',
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Customer Care Export');
            ...
            const fileName = `${selectedCategory?.name || 'customer_care_export'}_${dayjs().format('YYYY-MM-DD')}.xlsx`;
    ...
    const exportToWord = async () => {
        try {
            ...
            const tableRows = activities.map((activity, index) => {
                ...
                switch (selectedCategory?.id) {
                    case 'delivery_schedule':
                        cells.push(
                            new TableCell({
                                children: [new Paragraph({ children: [new TextRun(activity.region || '')] })]
                            }),
                            new TableCell({
                                children: [new Paragraph({ children: [new TextRun(activity.area || '')] })]
                            }),
                            new TableCell({
                                children: [new Paragraph({ children: [new TextRun(activity.delivery_date ? dayjs(activity.delivery_date).format('YYYY-MM-DD') : '')] })]
                            }),
                            new TableCell({
                                children: [new Paragraph({ children: [new TextRun(activity.duration || '')] })]
                            })
                        );
                        break;
                    ...
                    case 'meetings':
                        cells.push(
                            new TableCell({
                                children: [new Paragraph({ children: [new TextRun(activity.date ? dayjs(activity.date).format('YYYY-MM-DD') : '')] })]
                            }),
                            new TableCell({
            ...
            headerCells.push(
                ...
                new TableCell({ children: [new Paragraph({ children: [new TextRun('Priority')] })] }),
            ...
            switch (selectedCategory?.id) {
                case 'delivery_schedule':
                    headerCells.push(
                        new TableCell({ children: [new Paragraph({ children: [new TextRun('Region')] })] }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun('Area')] })] }),
                        new TableCell({ children: [new Paragraph({ children: [new TextRun('Delivery Date')] })] }),
                ...
                case 'meetings':
                    headerCells.push(
                        ...
                        new TableCell({ children: [new Paragraph({ children: [new TextRun('Subject')] })] })
                    );
                    break;
                ...
                sections: [{
                    children: [
                        new Paragraph({
                            children: [
                                new TextRun({
                                    text: `Customer Care Export - ${selectedCategory?.name}`,
                                    bold: true,
                                    size: 32,
                                }),
                            ],
            ...
            link.href = url;
            link.download = `${selectedCategory?.name || 'customer_care_export'}_${dayjs().format('YYYY-MM-DD')}.docx`;
...
const ExcelImportModal = ({ visible, onCancel, selectedCategory, onImportComplete }) => {
    ...
    const downloadTemplate = () => {
        try {
            ...
            const hasResponsibleBDM = selectedCategory?.hasResponsibleBDM;
            
            if (hasResponsibleBDM) {
                toast.error("We Cannot Create A Template For This Category, Because There Is A Mandatory Column Responsible BDM For Assign A Employee, Use Bulk Records To Add Multiple Records");
                return;
            }
            ...
            switch (selectedCategory?.id) {
                case 'delivery_schedule':
                    headers = ['Delivery Date*', 'Region*', 'Area*', 'Duration', 'Status', 'Priority*'];
                    templateData = [{
                ...
                case 'special_tasks':
                    ...
                    templateData = [{
                        'Date*': '15/01/2024',
                        'Subject*': 'Special Customer Support Task',
                        'Status': 'Pending',
                        'Priority*': '4'
                    }];
                    break;
            ...
            const fileName = `${selectedCategory?.name || 'customer_care'}_import_template.xlsx`;
            XLSX.writeFile(workbook, fileName);
    ...
    const validateExcelData = (data) => {
        ...
        data.forEach((row, index) => {
            ...
            switch (selectedCategory?.id) {
                case 'delivery_schedule':
                    const deliveryDate = row['Delivery Date'] || row['Delivery Date*'];
                    if (!deliveryDate) {
                        errors.push('Delivery Date is required');
                    } else {
                        const date = parseDate(deliveryDate);
                        if (!date || !date.isValid()) {
                            errors.push('Invalid delivery date format. Use DD/MM/YYYY (e.g., 15/01/2024) or YYYY-MM-DD');
                        } else {
                            validatedRow.delivery_date = date.format('YYYY-MM-DD');
                        }
                    }

                    if (!row.Region && !row['Region*']) errors.push('Region is required');
                    else validatedRow.region = row.Region || row['Region*'];
            ...
            if (selectedCategory?.id === 'delivery_schedule') {
                validatedRow.duration = row.Duration || '';
            }
            ...
            <Space direction="vertical" style={{ width: '100%' }} size="large">
                ...
                {hasResponsibleBDM && (
                    <Alert
                        ...
                        description="We Cannot Create A Template For This Category, Because There Is A Mandatory Column Responsible BDM For Assign A Employee, Use Bulk Records To Add Multiple Records"
                        type="warning"
                        showIcon
                    />
                )}
                ...
                <Card size="small" title="Upload Excel File" style={{ display: hasResponsibleBDM ? 'none' : 'block' }}>
                    <Upload.Dragger {...uploadProps} disabled={hasResponsibleBDM}>
                ...
                <Alert
                    ...
                    description={
                        <ul style={{ margin: 0, paddingLeft: '16px' }}>
                            ...
                            {hasResponsibleBDM && (
                                <li style={{ color: '#ff4d4f', fontWeight: 'bold' }}>
                                    Excel import is disabled for this category due to Responsible BDM requirement
                                </li>
                            )}
                        </ul>
                    }
...
const Customer_Care = () => {
    ...
    // Customer Care Categories configuration
    const customerCareCategories = [
        {
            id: 'delivery_schedule',
            name: 'Delivery Schedule',
            table: 'customer_care_delivery_schedule',
            type: 'Delivery Management',
            icon: <RocketOutlined />,
            dateField: 'delivery_date',
            color: '#1890ff',
            hasTimeFields: false,
            hasResponsibleBDM: true // This category has responsible_bdm column
        },
        {
            id: 'meetings',
            name: 'Meetings',
            table: 'customer_care_meetings',
            type: 'Customer Meetings',
            icon: <TeamOutlined />,
            dateField: 'date',
            color: '#fa8c16',
            hasTimeFields: false,
            hasResponsibleBDM: false
        },
        {
            id: 'special_tasks',
            name: 'Special Tasks',
            table: 'customer_care_special_tasks',
            type: 'Special Assignments',
            icon: <StarOutlined />,
            dateField: 'date',
            color: '#52c41a',
            hasTimeFields: false,
    ...
    const refreshCustomerCareData = async () => {
        ...
        try {
            ...
            toast.info('Customer Care data updated automatically');
        } catch (error) {
    ...
    const manualRefresh = async () => {
        ...
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.success('Customer Care data refreshed successfully');
        } catch (error) {
    ...
    // Initialize Customer Care module
    const initializeCustomerCare = async () => {
        setLoading(true);
        try {
            ...
            handleError(error, 'initializing Customer Care module');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchCustomerCareUsers = async () => {
        try {
            // Get all profiles (not just from current department)
            ...
            safeSetState(setCustomerCareUsers, allUsers || []);
        } catch (error) {
            handleError(error, 'fetching Customer Care users');
            safeSetState(setCustomerCareUsers, []);
        }
    };
    ...
    const fetchUserSchedule = async (userId, startDate, endDate) => {
        ...
        try {
            ...
            // Get user details
            const user = customerCareUsers.find(u => u.id === userId);
            if (!user) {
                console.warn('User not found in Customer Care users list');
            ...
            // 2. Customer Care activities
            for (const category of customerCareCategories) {
                ...
                try {
                    ...
                    if (data) {
                        // For Customer Care, we'll include all activities since there's no specific user assignment
        ...
        try {
            ...
            try {
                ...
                if (record.delivery_date) {
                    formattedRecord.delivery_date = safeDayjs(record.delivery_date);
                }
            } catch (dateError) {
    ...
    const handleBulkCreate = async (records) => {
        try {
            ...
            const validRecords = records.filter(record => {
                ...
                switch (selectedCategory.id) {
                    case 'delivery_schedule':
                        return record.delivery_date && record.region && record.area;
                    case 'meetings':
                    case 'special_tasks':
                    ...
                    return (
                        <>
                            ...
                            <Form.Item
                                ...
                                label="Duration"
                            >
                                <Input placeholder="Enter duration (e.g., 2 hours)" />
                            </Form.Item>
                        </>
                    );
                    ...
                    return (
                        <>
                            ...
                            <Form.Item
                                ...
                                rules={[{ required: true, message: 'Please enter subject' }]}
                            >
                                <TextArea rows={3} placeholder="Enter task subject" />
                            </Form.Item>
                        </>
                    );
    ...
    if (loading && !selectedCategory) {
        return <LoadingSpinner tip="Loading Customer Care module..." />;
    }
    ...
    return (
        <div style={{ padding: '16px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            {error && (
                <Alert
                    message="Customer Care Module Error"
                ...
                <Row justify="space-between" align="middle" gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={8}>
                        <Title level={2} style={{ margin: 0, fontSize: '24px' }}>
                            <TeamOutlined /> Customer Care Department
                        </Title>
                    </Col>
                    <Col xs={24} sm={12} md={8}>
                        <Space direction="vertical" style={{ width: '100%' }} size="small">
            ...
            {autoRefresh && (
                <Alert
                    ...
                    description="Customer Care data will automatically update every 2 minutes."
                    type="info"
                    showIcon
                    closable
                    style={{ marginBottom: 16 }}
            ...
            {/* Category Cards */}
            <Card
                title="Customer Care Categories"
                style={{ marginBottom: 24 }}
                extra={
                    <Tag color="blue">
                        {customerCareCategories.length} Categories Available
                    </Tag>
                }
            >
                <Row gutter={[16, 16]}>
            ...
            {selectedCategory && dateRange[0] && dateRange[1] && (
                <>
                    <CustomerCareStatistics stats={stats} loading={loading} />
            ...
            <Modal
                title={
                    <Space>
                        <UserOutlined />
                        Check Customer Care Team Availability
                        <Tag color="blue">
            ...
            >
                <Space direction="vertical" style={{ width: '100%' }} size="large">
                    ...
                    {/* Customer Care Users List */}
                    <Card size="small" title="Customer Care Team Members">
                        {customerCareUsers.length === 0 ? (
                            <Empty
                                image={Empty.PRESENTED_IMAGE_SIMPLE}
                                description="No Customer Care team members found"
                            />
                        ) : (
                            <List
                                dataSource={customerCareUsers}
                                renderItem={user => (
                                    <List.Item
                                        actions={[
                                            <Tooltip
                                        ...
                                        <List.Item.Meta
                                            ...
                                            description={
                                                <Space>
                                                    <Badge
                                                        status="success"
                                                        text="Available Team Member"
                                                    />
                                                    {user.department_id && (
                                                        <Tag color="blue">Department ID: {user.department_id}</Tag>
                                                    )}
                                                </Space>
                                            }
                                        />
                    ...
                    <Alert
                        ...
                        description={
                            <ul style={{ margin: 0, paddingLeft: '16px' }}>
                                ...
                                <li>All profiles from the system are shown, not just current department</li>
                            </ul>
                        }
                        type="info"
                        showIcon
            ...
            {/* Instructions */}
            {!selectedCategory && (
                <Card title="How to Use Customer Care Module" style={{ borderRadius: '12px' }}>
                    <Alert
                        message="Manage Customer Care Department Data"
                        description={
                            <div>
                                ...
                                <Text type="secondary">
                                    Each category represents different Customer Care activities recorded in the system.
                                    ...
                                    All profiles from the system are available for selection, not just current department.
                                </Text>
                            </div>
                        }
                        type="info"
