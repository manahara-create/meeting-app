...
const ErrorFallback = ({ error, resetErrorBoundary }) => (
    <Result
        status="error"
        title="Something went wrong in Cluster 1 Module"
...
// Loading component
const LoadingSpinner = ({ tip = "Loading Cluster 1 data..." }) => (
...
const Cluster1Statistics = ({ stats, loading = false }) => (
...
const DiscussionModal = React.memo(({
    ...
    // Get the correct feedback table name for Cluster 1
    const getFeedbackTable = useCallback(() => {
        if (!category) return null;

        const tableMap = {
            'visit_plan': 'cluster_1_visit_plan_fb',
            'meetings': 'cluster_1_meetings_fb',
            'special_task': 'cluster_1_special_task_fb'
        };
...
const UserScheduleModal = React.memo(({
    ...
    const getActivityDescription = (item) => {
        try {
            ...
            if (item.type === 'cluster_1_activity') {
...
const ExcelImportModal = ({ visible, onCancel, selectedCategory, onImportComplete, profiles }) => {
    ...
    const downloadTemplate = () => {
        try {
            ...
            const fileName = `${selectedCategory?.name || 'cluster_1'}_import_template.xlsx`;
...
const ExportButton = ({
    ...
    const exportToExcel = () => {
        try {
            const dataForExport = activities.map(activity => {
                // Common structure for Cluster 1 modules
                const base = {
            ...
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Cluster 1 Export');
            ...
            const fileName = `${selectedCategory?.name || 'cluster_1_export'}_${dayjs().format('YYYY-MM-DD')}.xlsx`;
    ...
    const exportToWord = () => {
        try {
            ...
            link.href = url;
            link.download = `${selectedCategory?.name || 'cluster_1_export'}_${dayjs().format('YYYY-MM-DD')}.doc`;
...
const Cluster1 = () => {
    ...
    const [cluster1Users, setCluster1Users] = useState([]);
    ...
    // Cluster 1 Categories configuration
    const cluster1Categories = [
        {
            id: 'visit_plan',
            name: 'Visit Plan',
            table: 'cluster_1_visit_plan',
            type: 'Visit Planning',
    ...
    const refreshCluster1Data = async () => {
        ...
        try {
            ...
            toast.info('Cluster 1 data updated automatically');
        } catch (error) {
    ...
    const manualRefresh = async () => {
        ...
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.success('Cluster 1 data refreshed successfully');
        } catch (error) {
    ...
    // Initialize Cluster 1 module
    const initializeCluster1 = async () => {
        setLoading(true);
        try {
            ...
            handleError(error, 'initializing Cluster 1 module');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchCluster1Users = async () => {
        try {
            // First get the Cluster 1 department ID
            ...
            if (deptError) {
                // If Cluster 1 department doesn't exist, try case-insensitive search
                const { data: deptDataAlt, error: deptErrorAlt } = await supabase
                    .from('departments')
                    .select('id')
                    .ilike('name', '%Cluster 1%')
                    .single();

                if (deptErrorAlt) throw deptErrorAlt;
                if (!deptDataAlt) {
                    toast.warning('Cluster 1 department not found. Using all users as fallback.');
                    ...
                    safeSetState(setCluster1Users, allUsers || []);
                    return;
                }

                safeSetState(setCluster1Users, []);
                return;
            }

            if (!deptData) {
                toast.warning('Cluster 1 department not found');
                safeSetState(setCluster1Users, []);
                return;
            }

            // Then get all users in Cluster 1 department
            ...
            if (usersError) throw usersError;
            safeSetState(setCluster1Users, usersData || []);
        } catch (error) {
            handleError(error, 'fetching Cluster 1 users');
            safeSetState(setCluster1Users, []);
        }
    };
    ...
    const fetchUserSchedule = async (userId, startDate, endDate) => {
        ...
        try {
            ...
            // Get user details
            const user = cluster1Users.find(u => u.id === userId);
            if (!user) {
                console.warn('User not found in Cluster 1 users list');
            ...
            // 2. Cluster 1 activities
            for (const category of cluster1Categories) {
                ...
                try {
                    ...
                    if (data) {
                        // For Cluster 1, we'll include all activities since there's no specific user assignment
    ...
    if (loading && !selectedCategory) {
        return <LoadingSpinner tip="Loading Cluster 1 module..." />;
    }
    ...
    return (
        <div style={{ padding: '16px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            {error && (
                <Alert
                    message="Cluster 1 Module Error"
                ...
                <Row justify="space-between" align="middle" gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={8}>
                        <Title level={2} style={{ margin: 0, fontSize: '24px' }}>
                            <TeamOutlined /> Cluster 1 Department
                        </Title>
                    </Col>
                    <Col xs={24} sm={12} md={8}>
                        <Space direction="vertical" style={{ width: '100%' }} size="small">
            ...
            {autoRefresh && (
                <Alert
                    ...
                    description="Cluster 1 data will automatically update every 2 minutes."
                    type="info"
                    showIcon
                    closable
                    style={{ marginBottom: 16 }}
            ...
            {/* Category Cards */}
            <Card
                title="Cluster 1 Categories"
                style={{ marginBottom: 24 }}
                extra={
                    <Tag color="blue">
                        {cluster1Categories.length} Categories Available
                    </Tag>
                }
            >
                <Row gutter={[16, 16]}>
                    {cluster1Categories.map((category) => (
            ...
            {selectedCategory && dateRange[0] && dateRange[1] && (
                <>
                    <Cluster1Statistics stats={stats} loading={loading} />
            ...
            <Modal
                title={
                    <Space>
                        <UserOutlined />
                        Check Cluster 1 Team Availability
                        <Tag color="blue">
            ...
            >
                <Space direction="vertical" style={{ width: '100%' }} size="large">
                    ...
                    {/* Cluster 1 Users List */}
                    <Card size="small" title="Cluster 1 Team Members">
                        {cluster1Users.length === 0 ? (
                            <Empty
                                image={Empty.PRESENTED_IMAGE_SIMPLE}
                                description="No Cluster 1 team members found"
                            />
                        ) : (
                            <List
                                dataSource={cluster1Users}
                                renderItem={user => (
                                    <List.Item
                                        actions={[
                                            <Tooltip
            ...
            {/* Instructions */}
            {!selectedCategory && (
                <Card title="How to Use Cluster 1 Module" style={{ borderRadius: '12px' }}>
                    <Alert
                        message="Manage Cluster 1 Department Data"
                        description={
                            <div>
                                ...
                                <Text type="secondary">
                                    Each category represents different Cluster 1 activities recorded in the system.
