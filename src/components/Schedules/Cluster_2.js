...
const ErrorFallback = ({ error, resetErrorBoundary }) => (
    <Result
        status="error"
        title="Something went wrong in Cluster 2 Module"
...
// Loading component
const LoadingSpinner = ({ tip = "Loading Cluster 2 data..." }) => (
...
const Cluster2Statistics = ({ stats, loading = false }) => (
...
const DiscussionModal = React.memo(({
    ...
    // Get the correct feedback table name for Cluster 2
    const getFeedbackTable = useCallback(() => {
        if (!category) return null;

        const tableMap = {
            'visit_plan': 'cluster_2_visit_plan_fb',
            'meetings': 'cluster_2_meetings_fb',
            'special_task': 'cluster_2_special_task_fb'
        };
...
const UserScheduleModal = React.memo(({
    ...
    const getActivityType = (item) => {
        try {
            ...
            if (item.type === 'cluster_2_activity') return `Cluster 2 ${item.activity_type}`;
            return 'Activity';
        } catch (error) {
            return 'Activity';
        }
    ...
    const getActivityDescription = (item) => {
        try {
            ...
            if (item.type === 'cluster_2_activity') {
...
const ExcelImportModal = ({ visible, onCancel, selectedCategory, onImportComplete, profiles }) => {
    ...
    const downloadTemplate = () => {
        try {
            ...
            const fileName = `${selectedCategory?.name || 'cluster_2'}_import_template.xlsx`;
                        ...
                        description={
                            <div>
                                ...
                                <ul style={{ margin: '8px 0', paddingLeft: '20px' }}>
                                    <li>Use Bulk Records feature to add multiple records with BDM assignments</li>
                                    <li>BDM assignments must be done manually in the system</li>
                                    <li>Excel import is not available for categories with Responsible BDM field</li>
                                </ul>
                            </div>
                        }
                        type="warning"
                ...
                <Alert
                    ...
                    description={
                        <ul style={{ margin: 0, paddingLeft: '16px' }}>
                            ...
                            {hasResponsibleBDM && (
                                <li><Text strong>This category requires BDM assignment - use Bulk Records instead</Text></li>
                            )}
                        </ul>
                    }
                    type="info"
...
const ExportButton = ({
    ...
    const exportToExcel = () => {
        try {
            const dataForExport = activities.map(activity => {
                // Common structure for Cluster 2 modules
                const base = {
            ...
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Cluster 2 Export');
            ...
            const fileName = `${selectedCategory?.name || 'cluster_2_export'}_${dayjs().format('YYYY-MM-DD')}.xlsx`;
    ...
    /** -----------------------------
     * Export to Word (.docx) - Simple implementation
     * ----------------------------- */
    const exportToWord = () => {
        try {
            ...
            let htmlContent = `
                ...
                <body>
                    <h1>Cluster 2 Export Report</h1>
                    <div class="summary">
                        <strong>Category:</strong> ${selectedCategory?.name || 'All'} <br>
                        <strong>Export Date:</strong> ${dayjs().format('DD/MM/YYYY HH:mm')} <br>
                        <strong>Total Records:</strong> ${activities.length}
                    </div>
            `;
            ...
            const columns = ['Priority', 'Date', 'Status'];
            switch (selectedCategory?.id) {
                case 'meetings':
            ...
            activities.forEach(activity => {
                ...
                columns.forEach(col => {
                    ...
                    switch (col) {
                        case 'Priority':
                            value = getPriorityLabel(activity.priority);
                            break;
                        case 'Date':
                            value = activity.date ? dayjs(activity.date).format('DD/MM/YYYY') : '';
                            break;
                        case 'Status':
            ...
            link.href = url;
            link.download = `${selectedCategory?.name || 'cluster_2_export'}_${dayjs().format('YYYY-MM-DD')}.doc`;
    ...
    /** -----------------------------
     * Dropdown menu - Updated to remove PDF and add Word
     * ----------------------------- */
    const exportItems = [
        {
            key: 'excel',
...
const Cluster2 = () => {
    ...
    const [cluster2Users, setCluster2Users] = useState([]);
    ...
    // Cluster 2 Categories configuration
    const cluster2Categories = [
        {
            id: 'visit_plan',
            name: 'Visit Plan',
            table: 'cluster_2_visit_plan',
            type: 'Visit Planning',
        ...
        },
        {
            id: 'meetings',
            name: 'Meetings',
            table: 'cluster_2_meetings',
            type: 'Meeting Management',
            icon: <TeamOutlined />,
            dateField: 'date',
            color: '#fa8c16',
            hasResponsibleBDM: true
        },
        {
            id: 'special_task',
            name: 'Special Task',
            table: 'cluster_2_special_task',
            type: 'Special Tasks',
            icon: <FlagOutlined />,
            dateField: 'date',
            color: '#52c41a',
    ...
    const refreshCluster2Data = async () => {
        ...
        try {
            ...
            toast.info('Cluster 2 data updated automatically');
        } catch (error) {
    ...
    const manualRefresh = async () => {
        ...
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.success('Cluster 2 data refreshed successfully');
        } catch (error) {
    ...
    // Initialize Cluster 2 module
    const initializeCluster2 = async () => {
        setLoading(true);
        try {
            ...
            handleError(error, 'initializing Cluster 2 module');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchCluster2Users = async () => {
        try {
            // First get the Cluster 2 department ID
            ...
            if (deptError) {
                // If Cluster 2 department doesn't exist, try case-insensitive search
                const { data: deptDataAlt, error: deptErrorAlt } = await supabase
                    .from('departments')
                    .select('id')
                    .ilike('name', '%Cluster 2%')
                    .single();

                if (deptErrorAlt) throw deptErrorAlt;
                if (!deptDataAlt) {
                    toast.warning('Cluster 2 department not found. Using all users as fallback.');
                    ...
                    safeSetState(setCluster2Users, allUsers || []);
                    return;
                }

                safeSetState(setCluster2Users, []);
                return;
            }

            if (!deptData) {
                toast.warning('Cluster 2 department not found');
                safeSetState(setCluster2Users, []);
                return;
            }

            // Then get all users in Cluster 2 department
            ...
            if (usersError) throw usersError;
            safeSetState(setCluster2Users, usersData || []);
        } catch (error) {
            handleError(error, 'fetching Cluster 2 users');
            safeSetState(setCluster2Users, []);
        }
    };
    ...
    const fetchUserSchedule = async (userId, startDate, endDate) => {
        ...
        try {
            ...
            // Get user details
            const user = cluster2Users.find(u => u.id === userId);
            if (!user) {
                console.warn('User not found in Cluster 2 users list');
            ...
            // 2. Cluster 2 activities
            for (const category of cluster2Categories) {
                ...
                try {
                    ...
                    if (data) {
                        // For Cluster 2, we'll include all activities since there's no specific user assignment
    ...
    const getFormFields = () => {
        ...
        try {
            ...
            // Add responsible_bdm field for categories that have it
            const responsibleBDMField = selectedCategory.hasResponsibleBDM ? (
                <Form.Item
                    name="responsible_bdm"
                    label="Responsible BDM"
                    rules={[{ required: true, message: 'Please select responsible BDM' }]}
                >
                    <Select
    ...
    if (loading && !selectedCategory) {
        return <LoadingSpinner tip="Loading Cluster 2 module..." />;
    }
    ...
    return (
        <div style={{ padding: '16px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            {error && (
                <Alert
                    message="Cluster 2 Module Error"
                ...
                <Row justify="space-between" align="middle" gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={8}>
                        <Title level={2} style={{ margin: 0, fontSize: '24px' }}>
                            <TeamOutlined /> Cluster 2 Department
                        </Title>
                    </Col>
                    <Col xs={24} sm={12} md={8}>
                        <Space direction="vertical" style={{ width: '100%' }} size="small">
            ...
            {autoRefresh && (
                <Alert
                    ...
                    description="Cluster 2 data will automatically update every 2 minutes."
                    type="info"
                    showIcon
                    closable
                    style={{ marginBottom: 16 }}
            ...
            {/* Category Cards */}
            <Card
                title="Cluster 2 Categories"
                style={{ marginBottom: 24 }}
                extra={
                    <Tag color="blue">
                        {cluster2Categories.length} Categories Available
                    </Tag>
                }
            >
                <Row gutter={[16, 16]}>
                    {cluster2Categories.map((category) => (
            ...
            {selectedCategory && dateRange[0] && dateRange[1] && (
                <>
                    <Cluster2Statistics stats={stats} loading={loading} />
            ...
            <Modal
                title={
                    <Space>
                        <UserOutlined />
                        Check Cluster 2 Team Availability
                        <Tag color="blue">
            ...
            >
                <Space direction="vertical" style={{ width: '100%' }} size="large">
                    ...
                    {/* Cluster 2 Users List */}
                    <Card size="small" title="Cluster 2 Team Members">
                        {cluster2Users.length === 0 ? (
                            <Empty
                                image={Empty.PRESENTED_IMAGE_SIMPLE}
                                description="No Cluster 2 team members found"
                            />
                        ) : (
                            <List
                                dataSource={cluster2Users}
                                renderItem={user => (
                                    <List.Item
                                        actions={[
                                            <Tooltip
            ...
            {/* Instructions */}
            {!selectedCategory && (
                <Card title="How to Use Cluster 2 Module" style={{ borderRadius: '12px' }}>
                    <Alert
                        message="Manage Cluster 2 Department Data"
                        description={
                            <div>
                                ...
                                <ol>
                                    ...
                                    <li>For categories with Responsible BDM field, use Bulk Records feature instead of Excel import</li>
                                </ol>
                                <Text type="secondary">
                                    Each category represents different Cluster 2 activities recorded in the system.
                                    ...
                                    Categories with Responsible BDM field require manual assignment and cannot use Excel import.
                                </Text>
                            </div>
                        }
                        type="info"
