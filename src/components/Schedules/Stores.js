...
import {
    ...
    UploadOutlined, FileAddOutlined, TruckOutlined, ShopOutlined, FileTextOutlined
...
// Department and Category IDs
const STORES_DEPARTMENT_ID = '001f7425-c458-4b8b-9158-ba8425c89c24';
const CATEGORY_IDS = {
    PLAN_LOADING: '2675b59b-4353-4ba1-9abe-99617321c47d',
    VST: '31abb01f-def2-4247-93ed-883a05fe50e1'
};
...
const ErrorFallback = ({ error, resetErrorBoundary }) => (
    <Result
        status="error"
        title="Something went wrong in Stores Module"
...
// Loading component
const LoadingSpinner = ({ tip = "Loading Stores data..." }) => (
...
const StoresStatistics = ({ stats, loading = false }) => (
...
const DiscussionModal = React.memo(({
    ...
    const getFeedbackTable = useCallback(() => {
        ...
        const tableMap = {
            'plan_loading': 'stores_plan_loading_fb',
            'vst': 'stores_vst_fb'
        };
                ...
                <Col span={12}>
                    ...
                    >
                        <TimePicker
                            ...
                            onChange={(time) => updateRecord(index, 'start_time', time ? time.format('HH:mm') : '')}
                            style={{ width: '100%' }}
                            format="HH:mm"
                ...
                <Col span={12}>
                    ...
                    >
                        <TimePicker
                            ...
                            onChange={(time) => updateRecord(index, 'end_time', time ? time.format('HH:mm') : '')}
                            style={{ width: '100%' }}
                            format="HH:mm"
...
// Excel Import Modal Component for Stores - Updated with consistent styling
const ExcelImportModal = ({ visible, onCancel, selectedCategory, onImportComplete, allProfiles, storesDepartmentId }) => {
    const [importLoading, setImportLoading] = useState(false);
    const [uploadedData, setUploadedData] = useState([]);
    const [validationResults, setValidationResults] = useState([]);

    // Check if category has responsible_bdm_ids field
    const hasResponsibleBDMField = selectedCategory?.hasResponsibleBDM;
    ...
    const downloadTemplate = () => {
        try {
            ...
            switch (selectedCategory?.id) {
                case 'plan_loading':
                case 'vst':
                    headers = ['Date*', 'Start Time', 'End Time', 'Cluster', 'Area', 'Priority*'];
                    templateData = [{
                        'Date*': '15/01/2024',
                        'Start Time': '09:00',
                        'End Time': '17:00',
            ...
            const fileName = `${selectedCategory?.name || 'stores'}_import_template.xlsx`;
    ...
    const validateExcelData = (data) => {
        ...
        data.forEach((row, index) => {
            ...
            const warnings = [];
            const validatedRow = {
                department_id: storesDepartmentId,
                ...
                <Alert
                    ...
                    description={
                        <ul style={{ margin: 0, paddingLeft: '16px' }}>
                            ...
                            <li>Time format: HH:mm (e.g., 09:00, 17:30)</li>
    ...
    const exportToExcel = () => {
        try {
            const dataForExport = activities.map(activity => {
                // Common structure for Stores modules
                const base = {
                    'Date': activity.date
            ...
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Stores Export');
            ...
            const fileName = `${selectedCategory?.name || 'stores_export'}_${dayjs().format('YYYY-MM-DD')}.xlsx`;
        ...
        try {
            ...
            const tableRows = activities.map((activity, index) => {
                const cells = [
                    ...
                    new TableCell({ children: [new Paragraph(activity.end_time || '')] }),
            ...
            const headers = ['Date', 'Start Time', 'End Time', 'Cluster', 'Area', 'Priority'];
                ...
                sections: [{
                    ...
                    children: [
                        new Paragraph({
                            children: [
                                new TextRun({
                                    text: `${selectedCategory?.name || 'Stores'} Export - ${dayjs().format('DD/MM/YYYY')}`,
                                    bold: true,
                                    size: 28,
                                }),
                            ],
            ...
            link.href = url;
            link.download = `${selectedCategory?.name || 'stores_export'}_${dayjs().format('YYYY-MM-DD')}.docx`;
...
const Stores = () => {
    ...
    // Updated Stores Categories configuration
    const storesCategories = [
        {
            id: 'plan_loading',
            name: 'Stores - Plan Loading',
            table: 'stores_plan_loading',
            type: 'Plan Loading',
            icon: <TruckOutlined />,
            dateField: 'date',
            color: '#1890ff',
            hasTimeFields: true,
            categoryId: CATEGORY_IDS.PLAN_LOADING,
    ...
    const refreshStoresData = async () => {
        ...
        try {
            ...
            toast.info('Stores data updated automatically');
        } catch (error) {
    ...
    const manualRefresh = async () => {
        ...
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.success('Stores data refreshed successfully');
        } catch (error) {
    ...
    const initializeStores = async () => {
        ...
        try {
            ...
            handleError(error, 'initializing Stores module');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchTableData = async () => {
        ...
        try {
            ...
            let query = supabase
                .from(selectedCategory.table)
                .select('*')
                .eq('department_id', STORES_DEPARTMENT_ID)
        ...
        try {
            ...
            const submitData = validRecords.map(record => {
                const preparedRecord = { 
                    ...record,
                    department_id: STORES_DEPARTMENT_ID,
                    category_id: selectedCategory.categoryId
                };
    ...
    if (loading && !selectedCategory) {
        return <LoadingSpinner tip="Loading Stores module..." />;
    }
        ...
        <div style={{ padding: '16px', maxWidth: '1400px', margin: '0 auto' }}>
                ...
                <Row justify="space-between" align="middle" gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={8}>
                        <Title level={2} style={{ margin: 0, fontSize: '24px' }}>
                            <ShopOutlined /> Stores Department
                        </Title>
                    </Col>
                    <Col xs={24} sm={12} md={8}>
                        <Space direction="vertical" style={{ width: '100%' }} size="small">
            ...
            {autoRefresh && (
                <Alert
                    ...
                    description="Stores data will automatically update every 2 minutes."
                    type="info"
                    showIcon
                    closable
                    style={{ marginBottom: 16 }}
            ...
            {/* Category Cards */}
            <Card
                title="Stores Categories"
                style={{ marginBottom: 24 }}
                extra={
                    <Tag color="blue">
                        {storesCategories.length} Categories Available
                    </Tag>
                }
            >
                <Row gutter={[16, 16]}>
                    {storesCategories.map((category) => (
            ...
            {selectedCategory && dateRange[0] && dateRange[1] && (
                <>
                    <StoresStatistics stats={stats} loading={loading} />
            ...
            <ExcelImportModal
                ...
                allProfiles={profiles}
                storesDepartmentId={STORES_DEPARTMENT_ID}
            />
            ...
            {/* Instructions */}
            {!selectedCategory && (
                <Card title="How to Use Stores Module" style={{ borderRadius: '12px' }}>
                    <Alert
                        message="Manage Stores Department Data"
                        description={
                            <div>
                                ...
                                <Text type="secondary">
                                    Each category represents different Stores activities recorded in the system.
