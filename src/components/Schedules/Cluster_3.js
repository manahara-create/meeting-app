...
const ErrorFallback = ({ error, resetErrorBoundary }) => (
    <Result
        status="error"
        title="Something went wrong in Cluster 3 Module"
...
// Loading component
const LoadingSpinner = ({ tip = "Loading Cluster 3 data..." }) => (
...
const Cluster3Statistics = ({ stats, loading = false }) => (
...
const DiscussionModal = React.memo(({
    ...
    // Get the correct feedback table name for Cluster 3
    const getFeedbackTable = useCallback(() => {
        if (!category) return null;

        const tableMap = {
            'visit_plan': 'cluster_3_visit_plan_fb',
            'meetings': 'cluster_3_meetings_fb',
            'special_task': 'cluster_3_special_task_fb'
        };
...
const UserScheduleModal = React.memo(({
    ...
    const getActivityType = (item) => {
        try {
            ...
            if (item.type === 'cluster_3_activity') return `Cluster 3 ${item.activity_type}`;
            return 'Activity';
        } catch (error) {
            return 'Activity';
        }
    ...
    const getActivityDescription = (item) => {
        try {
            ...
            if (item.type === 'cluster_3_activity') {
...
const ExcelImportModal = ({ visible, onCancel, selectedCategory, onImportComplete, profiles }) => {
    ...
    const hasResponsibleBDM = true; // Assuming all categories have this column
        ...
        try {
            ...
            switch (selectedCategory?.id) {
                ...
                case 'meetings':
                    ...
                    templateData = [{
                        'Date*': '15/01/2024',
                        'Subject*': 'Sample Meeting Subject',
                        'Status': 'Planned',
                        'Priority*': '2'
                    }];
                    break;
            ...
            const fileName = `${selectedCategory?.name || 'cluster_3'}_import_template.xlsx`;
    ...
    const handleFileUpload = (file) => {
        if (hasResponsibleBDM) {
            toast.warning("Excel import is not available for categories with 'Responsible BDM' column. Use bulk records instead.");
            return false;
        }
                        ...
                        description={
                            <div>
                                ...
                                <ul style={{ margin: '8px 0', paddingLeft: '20px' }}>
                                    <li>Use bulk records to add multiple records with BDM assignments</li>
                                    <li>BDM assignments must be done manually in the system</li>
                                    <li>Excel import is not available for this category</li>
                                </ul>
                            </div>
                        }
                        type="warning"
...
const ExportButton = ({
    ...
    const exportToExcel = () => {
        try {
            ...
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Cluster 3 Export');
            ...
            const fileName = `${selectedCategory?.name || 'cluster_3_export'}_${dayjs().format('YYYY-MM-DD')}.xlsx`;
            XLSX.writeFile(workbook, fileName);
    ...
    const exportToWord = async () => {
        try {
                ...
                sections: [{
                    ...
                    children: [
                        new Paragraph({
                            children: [
                                new TextRun({
                                    text: `Cluster 3 Export - ${selectedCategory?.name || 'All Categories'}`,
                                    bold: true,
                                    size: 32
                                })
                            ]
            ...
            link.href = url;
            link.download = `${selectedCategory?.name || 'cluster_3_export'}_${dayjs().format('YYYY-MM-DD')}.docx`;
...
const Cluster3 = () => {
    ...
    const [cluster3Users, setCluster3Users] = useState([]);
    ...
    // Cluster 3 Categories configuration
    const cluster3Categories = [
        {
            id: 'visit_plan',
            name: 'Visit Plan',
            table: 'cluster_3_visit_plan',
            type: 'Visit Planning',
    ...
    const refreshCluster3Data = async () => {
        if (isRefreshing) return;
        setIsRefreshing(true);
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.info('Cluster 3 data updated automatically');
        } catch (error) {
    ...
    const manualRefresh = async () => {
        ...
        try {
            await fetchTableData();
            safeSetState(setLastRefresh, new Date());
            toast.success('Cluster 3 data refreshed successfully');
        } catch (error) {
    ...
    // Initialize Cluster 3 module
    const initializeCluster3 = async () => {
        setLoading(true);
        try {
            ...
            handleError(error, 'initializing Cluster 3 module');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchCluster3Users = async () => {
        try {
            ...
            if (deptError) {
                const { data: deptDataAlt, error: deptErrorAlt } = await supabase
                    .from('departments')
                    .select('id')
                    .ilike('name', '%Cluster 3%')
                    .single();

                if (deptErrorAlt) throw deptErrorAlt;
                if (!deptDataAlt) {
                    toast.warning('Cluster 3 department not found. Using all users as fallback.');
                    ...
                    safeSetState(setCluster3Users, allUsers || []);
                    return;
                }
                safeSetState(setCluster3Users, []);
                return;
            }

            if (!deptData) {
                toast.warning('Cluster 3 department not found');
                safeSetState(setCluster3Users, []);
                return;
            }
            ...
            if (usersError) throw usersError;
            safeSetState(setCluster3Users, usersData || []);
        } catch (error) {
            handleError(error, 'fetching Cluster 3 users');
            safeSetState(setCluster3Users, []);
        }
    };
    ...
    const fetchUserSchedule = async (userId, startDate, endDate) => {
        ...
        try {
            ...
            const user = cluster3Users.find(u => u.id === userId);
            if (!user) {
                console.warn('User not found in Cluster 3 users list');
            ...
            for (const category of cluster3Categories) {
                let query = supabase
                    .from(category.table)
                    .select('*')
    ...
    if (loading && !selectedCategory) {
        return <LoadingSpinner tip="Loading Cluster 3 module..." />;
    }
    ...
    return (
        <div style={{ padding: '16px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            {error && (
                <Alert
                    message="Cluster 3 Module Error"
                ...
                <Row justify="space-between" align="middle" gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={8}>
                        <Title level={2} style={{ margin: 0, fontSize: '24px' }}>
                            <TeamOutlined /> Cluster 3 Department
                        </Title>
                    </Col>
                    <Col xs={24} sm={12} md={8}>
                        <Space direction="vertical" style={{ width: '100%' }} size="small">
            ...
            {autoRefresh && (
                <Alert
                    ...
                    description="Cluster 3 data will automatically update every 2 minutes."
                    type="info"
                    showIcon
                    closable
                    style={{ marginBottom: 16 }}
            ...
            {/* Category Cards */}
            <Card
                title="Cluster 3 Categories"
                style={{ marginBottom: 24 }}
                extra={
                    <Tag color="blue">
                        {cluster3Categories.length} Categories Available
                    </Tag>
                }
            >
                <Row gutter={[16, 16]}>
                    {cluster3Categories.map((category) => (
            ...
            {selectedCategory && dateRange[0] && dateRange[1] && (
                <>
                    <Cluster3Statistics stats={stats} loading={loading} />
            ...
            <Modal
                title={
                    <Space>
                        <UserOutlined />
                        Check Cluster 3 Team Availability
                        <Tag color="blue">
            ...
            >
                <Space direction="vertical" style={{ width: '100%' }} size="large">
                    ...
                    <Card size="small" title="Cluster 3 Team Members">
                        {cluster3Users.length === 0 ? (
                            <Empty
                                image={Empty.PRESENTED_IMAGE_SIMPLE}
                                description="No Cluster 3 team members found"
                            />
                        ) : (
                            <List
                                dataSource={cluster3Users}
                                renderItem={user => (
                                    <List.Item
                                        actions={[
                                            <Tooltip
            ...
            {/* Instructions */}
            {!selectedCategory && (
                <Card title="How to Use Cluster 3 Module" style={{ borderRadius: '12px' }}>
                    <Alert
                        message="Manage Cluster 3 Department Data"
                        description={
                            <div>
                                ...
                                <Text type="secondary">
                                    Each category represents different Cluster 3 activities recorded in the system.
